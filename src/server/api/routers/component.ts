...
// 权限模型（无登录环境下的归属隔离）
// - 登录用户：actorId = 用户 id
// - 匿名用户：actorId = "anon:" + 服务端签名的 HttpOnly cookie 中的 UUID
// - 只有组件归属者（authorId === actorId）才能修改/复制；匿名仅能浏览 PUBLIC 组件
// ---------------------------------------------------------------------------
type RouterCtx = { session?: { user?: { id?: string } | null } | null; anonymousId?: string | null };

const getActorId = (ctx: RouterCtx): string | null =>
  ctx.session?.user?.id ?? (ctx.anonymousId ? "anon:" + ctx.anonymousId : null);

// 匿名用户首次操作时，创建对应的 User 记录（Component.authorId 是外键，必须指向存在的 User）
async function ensureActor(ctx: RouterCtx & { db: typeof db }) {
...
// ---------------------------------------------------------------------------
// 简单内存限流（生产应换 Redis/持久化；这里按匿名 cookie 限流防刷额度）
// ---------------------------------------------------------------------------
const rateLimitMap = new Map<string, { count: number; resetAt: number }>();

function checkRateLimit(key: string | null | undefined, limit = 12, windowMs = 60_000) {
  if (!key) return;
  const now = Date.now();
...
export const componentRouter = createTRPCRouter({
  createComponent: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      checkRateLimit(ctx.anonymousId);
...
export const componentImportRouter = createTRPCRouter({
  importComponent: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      checkRateLimit(ctx.anonymousId, 5);
